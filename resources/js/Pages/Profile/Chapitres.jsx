...
import ChapitreBloc from "@/Components/UI/ChapitreBloc";
    ...
    return (
        <div className="relative h-full min-h-[calc(100vh-64px)]">
            ...
            <div className="flex items-center justify-between px-4 py-4">
                <h1 className="text-4xl">Mes chapitres</h1>
            ...
            {
                chapitres.data.length > 0 ?
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
                            {chapitres.data.map((chapitre) => (
                                <ChapitreBloc key={chapitre.id} chapitre={chapitre} canModify={true} />
                            ))}
                        </div>

                        <PaginationBar links={chapitres.meta.links} />
                    </>
                    :
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-4xl dark:text-gray-500">Aucun chapitre</div>
            }
        </div>
    );
}